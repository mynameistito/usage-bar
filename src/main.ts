...
async function fetchClaudeUsage() {
  ...
  try {
    ...
    if (errorMsg.includes('Credential not found') || errorMsg.includes('not found')) {
      errorMessage.textContent = 'Claude credentials not found. Please sign in to Claude Code first.';
...
function updateTimestamp(provider: 'claude' | 'zai') {
  ...
  const lastRefresh = provider === 'claude' ? claudeLastRefresh : zaiLastRefresh;
  if (!lastRefresh) {
