// All Tauri interactions are routed through SettingsCallbacks (see main.ts).
// Do NOT add direct invoke() calls here.
...
function createZaiConnectedState(
  callbacks: SettingsCallbacks,
  section: HTMLElement
): HTMLElement {
...
function createAmpConnectedState(
  callbacks: SettingsCallbacks,
  section: HTMLElement
): HTMLElement {
  const container = document.createElement("div");
...
function createAmpInputState(
  ...
  desc.className = "settings-credential-desc";
  desc.textContent =
    "Copy your session cookie from ampcode.com. Open DevTools → Application → Cookies → session. ";
