...
impl ClaudeService {
    ...
    pub async fn fetch_usage(client: Arc<reqwest::Client>) -> Result<UsageData> {
        ...
        match response.status() {
            ...
            StatusCode::FORBIDDEN => Err(anyhow!("Access denied — check your permissions")),
            StatusCode::TOO_MANY_REQUESTS => {
                Err(anyhow!("Rate limited — please wait and try again"))
            }
            status if status.is_server_error() => Err(anyhow!("Server error — try again later")),
            _ => Err(anyhow!("Failed to fetch usage data")),
        }
    }
    ...
    pub async fn fetch_tier(client: Arc<reqwest::Client>) -> Result<ClaudeTierData> {
        ...
        match response.status() {
            ...
            status if status.is_success() => Self::handle_tier_response(response).await,
            StatusCode::FORBIDDEN => Err(anyhow!("Access denied — check your permissions")),
            StatusCode::TOO_MANY_REQUESTS => {
                Err(anyhow!("Rate limited — please wait and try again"))
            }
            status if status.is_server_error() => Err(anyhow!("Server error — try again later")),
            _ => Err(anyhow!("Failed to fetch tier data")),
        }
    }
